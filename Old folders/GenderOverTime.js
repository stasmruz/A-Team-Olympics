...
d3.json(queryUrl).then(function (chart_data) {
  ...
  setTimeout(function(){init(searchResults)},500)

function init(theData){
  console.log("searchResults: ", theData);
  let selector = d3.select("#selDataset");
  let options = []
  const searchResultsList = Array.from(theData)
  default_filtered = searchResultsList.filter(result=>{
  ...
  default_filtered=searchResultsList.filter(result=>(result.Sport===selection));
  console.log(Object.entries(searchResultsList));
  const years= default_filtered.map(result=>result.Year);
  const labels=[]
  years.forEach(year=> {
      if (!labels.includes(year)){
